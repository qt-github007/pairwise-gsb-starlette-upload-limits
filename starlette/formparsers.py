...
def _validate_size_limit(name: str, value: int | None) -> int | None:
    if value is None:
        return None
...
class FormParser:
    def __init__(
        ...
        *,
        ...
        self.max_total_size = _validate_size_limit("max_total_size", max_total_size)
    ...
    def on_field_data(self, data: bytes, start: int, end: int) -> None:
        ...
        self._current_total_size += len(message_bytes)
        if self.max_total_size is not None and self._current_total_size > self.max_total_size:
            raise MultiPartException(
                f"Total upload size exceeded the maximum of {self.max_total_size} bytes.",
                status_code=413,
            )
        message = (FormMessage.FIELD_DATA, message_bytes)
        self.messages.append(message)
...
class MultiPartParser:
    ...
    def __init__(
        ...
        *,
        ...
        self.max_file_size = _validate_size_limit("max_file_size", max_file_size)
        self.max_total_size = _validate_size_limit("max_total_size", max_total_size)
    ...
    def on_part_data(self, data: bytes, start: int, end: int) -> None:
        ...
        if self._current_part.file is None:
            ...
            if self.max_file_size is not None and self._current_file_size > self.max_file_size:
                raise MultiPartException(
                    f"File exceeded the maximum size of {self.max_file_size} bytes.",
                    status_code=413,
                )
    ...
    async def parse(self) -> FormData:
        ...
        try:
            ...
            # Close all the files if there was an error, a client disconnect or a
            # cancellation, so no temporary file is left open. The original
